...
- 没有独立 `services` 层，业务逻辑仍大量分散在 router 和 repository 中。
- 起名请求仍同步调用 AI，没有进入 Worker 队列。
- Worker 仍只用于知识库文件处理，且使用 RabbitMQ，不是目标中的 ARQ + Redis。
...
| Services Layer | `AIService`、`OrderService`、`UserService` 等业务服务 | 未发现 `ainame/services` 目录或 Service 类 | 未符合 | 当前最大结构缺口 |
| Worker Layer | ARQ + Redis 处理 AI 慢请求和邮件任务 | 有 RabbitMQ + `rag_worker.py` 处理知识库文件 | 部分符合 | Worker 思路存在，但未覆盖 AI 起名慢请求 |
| Infrastructure Layer | PostgreSQL、Redis、DeepSeek、SQLAlchemy async | SQLAlchemy async、Redis、DeepSeek、RabbitMQ、MySQL、PostgreSQL checkpoint 都存在 | 部分符合 | 基础设施丰富，但主库和队列方案与目标不一致 |
...
- FastAPI 收到起名请求后，只把任务推送到 Redis 队列。
- 立即返回“任务已提交”。
...
- Worker 处理知识库文件解析。
- `rag_router.py` 将知识库任务投递到 RabbitMQ。
- `requirements.txt` 中有 `aio-pika`，没有 `arq`。
- 未发现 `enqueue_job`、`RedisSettings` 等 ARQ 典型代码。
- `/names/generate` 仍同步调用 `generate_naming_v2`。
- 验证码邮件仍在 HTTP 请求链路内发送。
...
- 核心 AI 起名慢请求没有进入队列。
- 没有 ARQ + Redis Worker。
- Redis 仍主要用于验证码，不是任务 Broker。
- 没有独立起名任务状态表。
- 没有任务结果查询接口或 WebSocket 推送。
...
- 短期可以复用 RabbitMQ，把起名任务也投递到 RabbitMQ。
- 如果坚持目标架构，则引入 ARQ + Redis，并逐步迁移知识库任务。
- 不建议长期同时维护 RabbitMQ 和 ARQ，除非有清晰边界。
...
Infrastructure 层比上一版更完整，但与目标仍有两个关键不一致：主业务库不是 PostgreSQL，Redis 没有承担任务队列。
...
目标是 ARQ + Redis，当前是 RabbitMQ + aio-pika。

需要明确：

- 是修改目标架构，正式采用 RabbitMQ？
- 还是按目标引入 ARQ + Redis？
...
- 起名任务也进入 RabbitMQ。
- 邮件任务也进入 RabbitMQ。
- 文档中将 Worker 层目标从 ARQ + Redis 改成 RabbitMQ。

路线 B：改为 ARQ + Redis

- 引入 `arq`。
- 起名、邮件、知识库任务逐步迁移到 ARQ。
- RabbitMQ 退出项目。

建议：团队小、任务量不复杂时，ARQ + Redis 更轻；如果已经决定长期使用 RabbitMQ，则应更新目标架构，不要两套并存。
...
3. 明确 RabbitMQ 与 ARQ + Redis 的队列选型。
